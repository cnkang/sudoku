...
test.describe('Accessibility E2E', () => {
  ...
  test('should expose screen-reader essentials for interactive regions', async ({ page }) => {
    await waitForAppReady(page);

    await expect(page.locator('#main-content')).toBeVisible();
    const liveRegionCount = await page.locator('[aria-live="polite"]').count();
    expect(liveRegionCount).toBeGreaterThan(0);
    await expect(page.locator('#difficulty-select:visible').first()).toHaveAttribute(
      'aria-label',
      /difficulty/i,
    );
  ...
  test('should render visible focus indicators for keyboard users', async ({ page }) => {
    await waitForAppReady(page);

    const skipLink = page.locator('.skip-to-content');
    await skipLink.evaluate((el: HTMLElement) => el.focus());
    const hasFocus = await skipLink.evaluate((el) => document.activeElement === el);
    ...
    expect(hasVisibleOutline || hasVisibleShadow || isMovedIntoViewport).toBe(true);
  });
  ...
  test('should respect reduced-motion preference', async ({ page }) => {
    ...
    const reducedMotionStyles = await page.evaluate(() => {
      ...
      return {
        mediaMatches: globalThis.matchMedia('(prefers-reduced-motion: reduce)').matches,
