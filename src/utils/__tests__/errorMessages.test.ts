...
describe('errorMessages', () => {
  describe('VALIDATION_ERRORS', () => {
    it('should have all required validation error messages', () => {
      expect(VALIDATION_ERRORS.INVALID_GRID_SIZE).toBe('Invalid grid size. Must be 4, 6, or 9.');
      expect(VALIDATION_ERRORS.INVALID_DIFFICULTY_RANGE).toBe(
        'Invalid difficulty level. Must be between 1 and 10.',
      );
      expect(VALIDATION_ERRORS.DIFFICULTY_REQUIRED).toBe('Difficulty must be a valid number.');
  ...
  describe('createValidationError', () => {
    ...
    it('should create dynamic grid error message', () => {
      expect(createValidationError.invalidGrid(9)).toBe('Invalid grid: must be a 9x9 array.');
      expect(createValidationError.invalidGrid(4)).toBe('Invalid grid: must be a 4x4 array.');
    });
    ...
    it('should create dynamic cell position error message', () => {
      expect(createValidationError.invalidCellAt(2, 3, 'Value out of range')).toBe(
        'Invalid cell at [2, 3]: Value out of range',
      );
    });
  });
  ...
  describe('API_ERRORS', () => {
    it('should have all required API error messages', () => {
      ...
      expect(API_ERRORS.OFFLINE_MESSAGE).toBe('Offline - please try again when connected');
      expect(API_ERRORS.SYNC_PROGRESS_FAILED).toBe('Failed to sync progress');
      expect(API_ERRORS.SYNC_ACHIEVEMENT_FAILED).toBe('Failed to sync achievement');
    });
  });
  ...
  describe('SW_ERRORS', () => {
    it('should have all required Service Worker error messages', () => {
      ...
      expect(SW_ERRORS.INVALID_SOURCE_URL).toBe('Ignoring message with invalid source URL');
  ...
  describe('HOOK_ERRORS', () => {
    it('should have all required hook error messages', () => {
      ...
      expect(HOOK_ERRORS.MEDIA_QUERY_NOT_SUPPORTED).toBe('Media queries not supported');
    });
  });

  describe('UTILITY_ERRORS', () => {
    it('should have static utility error messages', () => {
      expect(UTILITY_ERRORS.DEFAULT_THEME_NOT_FOUND).toBe('Default theme not found');
    ...
    it('should create dynamic utility error messages', () => {
      expect(UTILITY_ERRORS.UNSUPPORTED_GRID_SIZE(5)).toBe('Unsupported grid size: 5');
      expect(UTILITY_ERRORS.UNSUPPORTED_GRID_SIZE(10)).toBe('Unsupported grid size: 10');
